...
namespace TPinfo
{
    class OrientedGraph
    {
        ...
        public OrientedGraph(int size, int def)
        {
            _size = size;
            _def = def;
        ...
        public OrientedGraph GetStronglyConnexGraph(int s)
        {
            List<int> sVertices = BFSOrder(s);
            List<int> vertices = new List<int>(sVertices.Count);
            for (int i = 0; i < sVertices.Count; i++)
            {
                List<int> iVertices = BFSOrder(i);
                if (iVertices.Contains(s)) vertices.Add(s);
            }
            OrientedGraph graph = new OrientedGraph(vertices.Count, _def);
